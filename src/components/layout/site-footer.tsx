...
const footerLinkClass =
  "block py-0.5 leading-snug text-slate-100/90 [overflow-wrap:anywhere] transition-colors hover:text-white focus-visible:outline-none focus-visible:underline";
    ...
    >
      <div className="mx-auto max-w-[120rem] min-w-0 px-4 pb-[calc(5.5rem+env(safe-area-inset-bottom,0px))] pt-8 sm:px-6 sm:pb-8 lg:px-10 lg:pt-10">
        <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-12 lg:gap-8">
          ...
          {/* Our services — compact single list (mobile/tablet) */}
          <nav aria-label="Our services" className="sm:col-span-1 lg:hidden">
            ...
            <ul className="mt-2 flex flex-col text-sm">
              {services.map((s) => (
                <li key={s.slug}>
                  <Link href={`/services/${s.slug}`} className={footerLinkClass}>
                    {s.title}
                  </Link>
                </li>
              ))}
              <li>
                <Link
                  href="/services"
                  className={cn(footerLinkClass, "font-medium text-amber-300 hover:text-amber-200")}
                >
                  View all services
                </Link>
              </li>
          ...
          {/* Our services — one heading, multi-column links on desktop */}
          <nav aria-label="Our services" className="hidden lg:col-span-6 lg:block">
            <h2 className="text-sm font-semibold uppercase tracking-wide text-slate-900/85">Our services</h2>
            <ul className="mt-2 grid grid-cols-3 gap-x-6 text-sm">
              {services.map((s) => (
                <li key={s.slug}>
                  <Link href={`/services/${s.slug}`} className={footerLinkClass}>
                    {s.title}
